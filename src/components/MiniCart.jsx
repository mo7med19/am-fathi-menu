      ...
      <aside className={`mini-cart ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="cart-body">
          {cart.length === 0 ? (
            ...
            cart.map((item) => {
              const itemCurrency = item.currency || menu.currency || "EGP";
                  ...
                  <div className="cart-info">
                    ...
                    {hasAddons && (
                      <div className="cart-addons">
                        <small className="cart-addons-title">
                          الإضافات:
                        </small>

                        {addons.map((addon) => (
                          <small key={addon.id} className="cart-addon-line">
                            {addon.name} +{" "}
                            {formatMoney(addon.price, itemCurrency)}
                          </small>
                        ))}
                      </div>
                    )}
                    ...
                    {hasAddons && typeof item.basePrice !== "undefined" && (
                      <span>
                        السعر الأساسي:{" "}
                        {formatMoney(item.basePrice, itemCurrency)}
                      </span>
                    )}
